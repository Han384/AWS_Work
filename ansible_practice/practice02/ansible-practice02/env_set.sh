...
# RDSインスタンスの情報を取得し、RDSエンドポイントを抽出して環境変数に設定、~./bash_profileに追記
...
# アクセスキー・シークレットアクセスキーの情報を取得して環境変数に設定、~./bash_profileに追記
...
# リージョン情報を取得して環境変数に設定、~./bash_profileに追記
echo 'export AWS_S3_REGION=$(aws configure get region)' >> ~/.bash_profile

# S3バケット名を環境変数に設定、~./bash_profileに追記
